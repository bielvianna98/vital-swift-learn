...
import { ProcedimentoEmDesenvolvimento } from "@/components/ProcedimentoEmDesenvolvimento";
import { getProcedimento } from "@/lib/procedimentos";
import {
  AlertTriangle,
  Baby,
  BookOpen,
...
export const Route = createFileRoute("/teorica/$slug")({
  head: ({ params }) => {
    const p = getProcedimento(params.slug);
    return {
      meta: [
        { title: `${p?.nome ?? "Procedimento"} — Aula Teórica — MedStep` },
        {
          name: "description",
          content: p?.descricao ?? "Aula teórica do procedimento.",
        },
      ],
    };
  },
  loader: ({ params }) => {
    const p = getProcedimento(params.slug);
    if (!p) throw notFound();
    return { procedimento: p };
  },
  component: TeoricaSlug,
});
...
function TeoricaSlug() {
  const { procedimento } = Route.useLoaderData();

  if (procedimento.slug !== "desengasgo-bebes") {
    return (
      <AppShell title="Aula Teórica" showBack>
        <ProcedimentoEmDesenvolvimento
    ...
    <AppShell title="Aula Teórica" showBack>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          Conheça os fundamentos teóricos, sinais clínicos e cuidados essenciais antes
          de praticar.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <Card icon={BookOpen} title="O que é">
          <p>
            Conjunto de manobras de desobstrução de vias aéreas indicadas para
            lactentes (menores de 1 ano) com obstrução por corpo estranho. Combina{" "}
            <strong>tapas interescapulares</strong> e{" "}
        ...
        <Card icon={Baby} title="Anatomia relevante">
          <p>
            A via aérea do lactente é estreita e flexível. A epiglote é
            proporcionalmente maior e a traqueia mais curta — pequenos objetos
            obstruem rapidamente. Sustentar sempre cabeça e mandíbula durante a
            manobra.
          </p>
        </Card>
        ...
        <Card icon={ShieldCheck} title="Cuidados importantes">
          <ul className="list-disc pl-5 space-y-1">
            <li>
              <strong>Nunca</strong> realizar varredura digital às cegas.
            </li>
        ...
        <Card icon={Stethoscope} title="Conduta resumida">
          <p>
            Avalie consciência → posicione decúbito ventral →{" "}
            <strong>5 tapas</strong> interescapulares → vire em decúbito dorsal →{" "}
            <strong>5 compressões</strong> torácicas → reavalie. Repetir até
            desobstrução ou perda de consciência.
          </p>
        </Card>
      </div>
