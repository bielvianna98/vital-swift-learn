...
export const Route = createFileRoute("/pratica/$slug")({
  head: ({ params }) => {
    const p = getProcedimento(params.slug);
    return {
      meta: [
        { title: `${p?.nome ?? "Procedimento"} — Aula Prática — MedStep` },
        {
          name: "description",
          content: p?.descricao ?? "Aula prática do procedimento.",
        },
      ],
    };
  },
  ...
  return (
    <AppShell title="Aula Prática" showBack>
      ...
      <div className="aspect-video rounded-2xl bg-primary/95 flex items-center justify-center shadow-glow relative overflow-hidden mb-6">
        ...
        <span className="absolute bottom-3 left-4 text-white/80 text-xs">
          Vídeo demonstrativo · 2:14
        </span>
      </div>
      ...
      <ol className="flex flex-col gap-3">
        {steps.map(({ n, icon: Icon, title, text }) => (
          <li
            key={n}
            className="bg-card rounded-2xl border border-border/60 shadow-card p-4 flex gap-4"
          >
